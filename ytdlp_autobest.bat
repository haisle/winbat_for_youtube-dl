...
goto:both
:both
	yt-dlp -f bestvideo+bestaudio --yes-playlist --write-thumbnail --merge-output-format mkv --download-archive "%~dp0\ytdlpdownloaded_list.txt" %url% -o "%~dp0combined\%%(playlist)s\%%(channel)s-%%(channel_id)s\%%(title)s-%%(upload_date)s-%%(id)s.%%(ext)s"
goto:fin
:fin
